...
def register():
    if request.method == 'POST':
        ...
        if existing_user:
            flash("Email is already registered. Try logging in instead.", "error")
        ...
        try:
            cursor.execute("""
                INSERT INTO users (student_id, password, course_name, full_name, mobile, email, unit_area, address, role)
...
def approve_leave(leave_id, action):
    ...
    cursor.execute("SELECT student_id FROM leave_applications WHERE id = %s", (leave_id,))
    res = cursor.fetchone()
    if not res:
